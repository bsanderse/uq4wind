function [x, w, err, n] = calibrate_linear(u, varargin)
...
likelihood = @(x) exp(- sum((u(x,A)-z).^2, 2) ./ sigma );
posterior = @(x) likelihood(x).*prior(x);

%% Construct (approximation) of true mean using MC
X = rand(S, d);
evidence = mean(posterior(X));
truemean = mean(X.*posterior(X)) / evidence;
clear X
...
[x, w, err, n] = analyticnd(d, posterior, ...
    'truemean', truemean, ...
    'I', I, ...
    varargin_analyticnd{:});
