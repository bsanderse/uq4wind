...
%     calculates the Cumulative Distribution Function (CDF) of samples of a
%     random vector collected in X matrix. Each column of X corresponds to
%     a component of the random vector that follows some distribution with
%     some parameters as specified by the Marginals structure array
%
...
for ii = 1:length(Marginals)
    ...
    if strcmpi(Marginals(ii).Type, 'ks')
        if ~uq_isnonemptyfield(Marginals(ii), 'Parameters') && isfield(Marginals(ii), 'KS') 
            ...
            Marginals(ii) = rmfield(Marginals(ii), 'KS');
        else
