...
for ii = 1 : length(scaling)
    ...
    %% Custom Trend
    rng(s);
    metaopts.Name = ['Custom_Trend',num2str(modelID)];
    metaopts.Trend.Type = 'custom' ;
    metaopts.FullModel = 'y_trig';
    metaopts.Trend.CustomF = 'cos';
    [~,Custom_Krig] = evalc('uq_createModel(metaopts)');
...
%     metaopts.Trend.PolyTypes = 'auto' ;
    ...
    Y_custom = uq_evalModel(uq_getModel(['Custom_Trend',num2str(modelID)]),Xpred);
    ...
    pass = pass & max(abs(Y_custom - Y_full_trig)) < eps;
