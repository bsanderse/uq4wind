...
root_folder = pwd;
filename = fullfile('AEROmodule','NewMexico','unperturbed935','output','B1n_BEM.txt');
...
%     Fn_mean = mean(Fn);
    ...
    figure(10)
    plot(azi_last_rev,Fn_pert,'-','Color',colormap(k,:), 'LineWidth',1);
    hold on
    plot(azi_last_rev,Fnew,'--','Color',colormap(k,:), 'LineWidth',2);
    % alternative, we can plot in sine-cosine basis:
%     plot(azi_last_rev,real(f_keep(1))+2*real(f_keep(2))*cos(2*pi*azi_last_rev/360)-2*imag(f_keep(2))*sin(2*pi*azi_last_rev/360))
