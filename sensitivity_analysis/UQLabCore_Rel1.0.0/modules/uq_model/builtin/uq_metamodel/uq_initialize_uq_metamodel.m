...
%% MANUAL DEFINITION OF KRIGING(predictor only)
% in case we want to manually specify the kriging parameters and use
% predictor without calculating, provide all the necessary information
...
if isfield(Options.ExpDesign, 'Preproc')
    ...
    if isfield(Options.ExpDesign.Preproc, 'PreY')
        ...
        switch class(Options.ExpDesign.Preproc.PreY)
            case 'function_handle'
                %current_model.Internal.ExpDesign.PreprocY = Options.ExpDesign.Preproc.PreY;
        ...
        if isfield(Options.ExpDesign.Preproc,'PreYPar')
            ...
            current_model.Internal.ExpDesign.PreYPar = Options.ExpDesign.Preproc.PreYPar;
        end
        postYopts.isVectorized = 1;
        current_model.Internal.ExpDesign.PreprocY = uq_createModel(preYopts, '-private');
    ...
    if isfield(Options.ExpDesign.Preproc, 'PostY')
        ...
        current_model.Internal.ExpDesign.PostprocY = uq_createModel(postYopts, '-private');
    end
end
...
if USER_SPECIFIED_ED
    ...
    switch lower(current_model.ExpDesign.Sampling)
        case {'lhs', 'mc', 'sobol', 'halton', 'simplelhs','grid'}
