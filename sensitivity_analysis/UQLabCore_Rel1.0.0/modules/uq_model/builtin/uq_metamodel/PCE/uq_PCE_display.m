...
for ii = 1:length(outArray)
    ...
    for k=1:4
        ...
        pp = [pp uq_plot(Indices(Color_k), logCoefs(Color_k),...
            'o','Color',MyColors(k,:), 'MarkerSize',(15-3*k))];
        hold on
    end
    Color_k = find(PsiDegrees>3);
    pp = [pp uq_plot(Indices(Color_k), logCoefs(Color_k),...
        'o','Color',MyColors(6,:), 'MarkerSize',3)];
    
    % layout and legend
    logCoefRange = max(1,max(logCoefs) - min(logCoefs));
    set(gca,'xlim', [-0.05*length(Coefs),1.05*length(Coefs)]);
    set(gca,'ylim',...
        [min(logCoefs)-0.05*logCoefRange, max(logCoefs)+0.05*logCoefRange]);
    set(gca,'YTickMode','manual');
    set(gca, 'YTick', unique(round(linspace(min(logCoefs),1, 5))));
    ...
    MyTitle = title(sprintf('NNZ coefs: %d',length(PsiDegrees)));
    xlabel('$\alpha$', 'FontSize', 16);
    ylabel('$\mathrm{\log_{10}\left(\left|y_{\alpha}\right|\right)}$',...
        'FontSize', 16);

    LegEntries = {'Mean', '$p=1$', '$p=2$', '$p=3$', '$p>3$'};
    MyLegend = uq_legend(pp,...
        LegEntries{1:min(length(pp),length(LegEntries))});
    set(MyLegend, 'Interpreter', 'latex', 'FontSize', 14);
