...
if isempty(Xred); error('Only constants in the input model, no meta-modelling required.'); end


% and a corresponging reduced input space
redopts.Marginals = current_model.Internal.Input.Marginals(current_model.Internal.Input.nonConst);
for ii = 1:length(current_model.Internal.Input.nonConst)
    redopts.Marginals(ii).Moments = [];
end
redopts.Copula = current_model.Internal.Input.Copula;
redopts.Copula.Parameters = redopts.Copula.Parameters(:,current_model.Internal.Input.nonConst);
redopts.Copula.Parameters = redopts.Copula.Parameters(current_model.Internal.Input.nonConst,:);

RedInput = uq_createInput(redopts, '-private');
