% UQ_KRIGINGOPTIONS displays a helper for the main options needed to
% create a Kriging metamodel in UQLab
...
disp('    KRIGINGOPTIONS.Optim.MaxIter = 10');
disp('    KRIGINGOPTIONS.Optim.Tol = 1e-3');
disp('    KRIGINGOPTIONS.Optim.Display = ''None''');
disp('    KRIGINGOPTIONS.Optim.HGA.nLM = 5');
disp('    KRIGINGOPTIONS.Optim.HGA.nPop = min(20, floor(4+3*log(M)) )');
disp('    KRIGINGOPTIONS.Optim.HGA.nStall = 2');
disp('    KRIGINGOPTIONS.Scaling = 1');
disp(' ')
disp('Please refer to the Kriging User Manual (<a href="matlab:uq_doc(''Kriging'',''html'')">HTML</a>,<a href="matlab:uq_doc(''Kriging'',''pdf'')">PDF</a>) for more detailed');
