...
CorrFamilies = {'exponential','linear','gaussian','matern-3_2','matern-5_2', 'handle'};
CorrTypes = {'Separable','Ellipsoidal'};
IsotropyCases = {true, false};
optMethods = {'bfgs', 'hga','hsade'} ;
estMethods = {'ml','cv'};
...
for ii = 1:length(avoid_BFGS_optim)
   combIdx(combIdx(:,1)==avoid_BFGS_optim(ii) & combIdx(:,4) == 1,:) = []; 
end
%combIdx = combIdx(accidx,:);

% for normal level randomly pick 30 cases
if strcmpi(level, 'normal')
   randidx = randperm(size(combIdx,1),30);
...
for ii = 1 : size(combIdx,1)
    ...
    metaopts.Optim.(upper(optMethods{curr_idx(4)})).nStall = 4;
    metaopts.Optim.(upper(optMethods{curr_idx(4)})).nLM = 10;
    metaopts.Optim.(upper(optMethods{curr_idx(4)})).nPop = 40;
...
function theta_star = compute_true_theta_star(krgmdl)
    ...
    switch lower(krgmdl.Internal.Kriging(1).GP.EstimMethod)
        ...
        case 'cv'
            ...
            parameters.CV_K = krgmdl.Internal.Kriging(1).GP.CV.CV_K;
    end
