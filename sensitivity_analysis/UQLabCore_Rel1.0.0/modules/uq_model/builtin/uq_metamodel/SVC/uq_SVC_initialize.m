...
%     any(strcmpi(class(current_model.Internal.Input),{'uq_input','struct'}));
% 
% if INPUT_EXISTS
%     if any(strcmpi(Options.ExpDesign.Sampling, {'user', 'data'}))
...
%     % Create a pseudo-INPUT object (that is actually a struct)
%     if any(strcmpi(current_model.ExpDesign.Sampling, {'data','user'}))
%         [Marginals(1:M).Type] = deal('data') ;
%         for oo = 1:M
%             mu = mean(current_model.ExpDesign.X(:,oo),1);
%             stDev = std(current_model.ExpDesign.X(:,oo),0,1);
%             Marginals(oo).Moments = [mu,stDev] ;
%             Marginals(oo).Parameters = Marginals(oo).Moments ;
%         end
%         EDInput = [];
%         EDInput.Marginals = Marginals;
%         EDInput.Copula.Type = 'Independent';
%         current_model.Internal.Input = EDInput;
%     end
% end
%% INPUT 
% Check whether an INPUT object has been defined
