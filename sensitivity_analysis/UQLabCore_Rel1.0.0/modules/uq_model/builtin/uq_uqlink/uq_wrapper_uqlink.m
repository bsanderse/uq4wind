    ...
    if strcmpi(action, 'recover')
        ...
        if ~ischar(RecoverySource) || isnumeric(RecoverySource)
            ...
            if any(RecoverySource > size(X,1))
                error('The list of runs contains IDs larger than the isze of ''X''!');
            end
...
for ii = RunList
    ...
    if num_of_out_args > 1
        save(P.Runtime.Processed , 'uq_ProcessedX') ;
        ...
        save(P.Runtime.Processed , 'uq_ProcessedX','uq_ProcessedY') ;
    end
