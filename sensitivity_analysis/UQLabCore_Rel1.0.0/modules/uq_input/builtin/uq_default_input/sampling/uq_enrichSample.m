...
uNew(:,indConst) = 0.5 * ones(N,length(indConst)) ;
...
%% map the samples from u (uniform space) to X (physical space)

switch lower(current_input.Copula.Type)
    case 'independent'
        %  If marginals are independent: directly perform the isop. transformation
        %  from u to X 
        Xnew = uq_IsopTransform(uNew, uMarginals, current_input.Marginals);
    case 'gaussian'
        % Assign marginals Types of  U marginals(standard normal) that correspond
        %to non-constant marginals
        [UMarginals(indNonConst).Type] = deal('gaussian') ;
        % Assign marginals Parameters of  U marginals(standard normal) that correspond
        %to non-constant marginals
        [UMarginals(indNonConst).Parameters] = deal([0 1]) ;
        % Assign marginals Types of U marginals that correspond to constant
        % marginals
        [UMarginals(indConst).Type] = deal('constant') ;
        % Assign marginals Parameters of U marginals that correspond to constant
        % marginals
        ...
        Unew = uq_IsopTransform(uNew, uMarginals, UMarginals);
        % Then map: U -> X(physical space) using inverse Nataf transformation
        Xnew = uq_invNatafTransform(Unew, current_input.Marginals, current_input.Copula);
    otherwise
        %  If marginals dependency is unknown throw an error
