...
copula_parameters = zeros(length(IOpts.Marginals));
nonConst = ~ismember(lower({IOpts.Marginals.Type}),{'constant'});
copula_parameters(find(nonConst),find(nonConst)) = [1, Rho; Rho, 1];
IOpts.Copula.Parameters = copula_parameters;
