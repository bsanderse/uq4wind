...
%   function that evaluates the forward models.
%
...
for ii = 1:Internal.nForwardModels
    ...
    modelParamsCurr = modelParams(:,Internal.ForwardModel(ii).PMap);
    forwardModel(ii).evaluation = uq_evalModel(Internal.ForwardModel(ii).Model,modelParamsCurr);
end
...
%% loop over error groups and fill likelihood and logLikelihood vector
...
for ii = 1:Internal.nDataGroups
    ...
    % fill modelRunsCurr
    modelRunsCurr = zeros(size(x,1),length(OutputIDCurr));
    for jj = 1:Internal.nForwardModels
    ...
    if discrepancyCurr.ParamKnown % known parameters
        errorParam = discrepancyCurr.Parameters;
    else
    ...
    switch type % what functions to evaluate
        case 'Likelihood'
            likelihood_curr = discrepancyCurr.likelihood_g(modelRunsCurr,errorParam);
            out = out.*likelihood_curr;
        case 'LogLikelihood'
            logLikelihood_curr = discrepancyCurr.logLikelihood_g(modelRunsCurr,errorParam);
