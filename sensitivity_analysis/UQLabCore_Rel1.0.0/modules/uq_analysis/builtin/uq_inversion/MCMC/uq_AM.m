...
%
%   See also: UQ_INVERSION, UQ_PLOTMCMC, UQ_AIES, UQ_MH, UQ_HMC
...
for ii = 2:Steps
    ...
    for jj = 1:nChains
        ...
        %update covariance to iith
        %biased (1/N) recursive covariance estimator
        t = ii-1;
        C_ad(:,:,jj) = C_ad(:,:,jj)*(t-1)/t + ...
            sd*(t-1)/(t^2)*(...
            (currSample-prevSampleMean)*(currSample-prevSampleMean)');
    end
    ...
    if ii < T0
        ...
        for jj = 1:nChains
            try
                ...
                C_eps = C_ad(:,:,jj) + eps*diag(diag(C_ad(:,:,jj)));
