...
%   LOGPRIOR, LOGLIKELIHOOD, VISUALIZE, STOREMODEL) samples from the posterior 
%   distribution defined by the product: LOGPRIOR*LOGLIKELIHOOD. The
%   algorithm tuning parameters are the number of leapfrog integrator steps
%   LEAPFROGSTEPS, step size LEAPFROGSIZE and the mass matrix MASS. 
...
%   SAMPLE is a 3D array containing the produced sample points.
%   
%   [SAMPLE, ACCEPT, TIME, FORWARDMODEL, LOGLIKELIEVAL] = UQ_HMC(STEPS, 
%   LEAPFROGSTEPS, LEAPFROGSIZE, MASS, SEED, LOGPRIOR, LOGLIKELIHOOD, 
%   VISUALIZE, STOREMODEL) additionally returns the acceptance rate ACCEPT 
%   per chain, the total required execution time TIME, the forward model 
%   runs FORWARDMODEL and the corresponding log-likelihood evaluations 
%   LOGLIKELIEVAL.
%
%   References:
%
...
%
%   See also: UQ_INVERSION, UQ_PLOTMCMC, UQ_AM, UQ_MH, UQ_AIES                                    
...
for ii = 2:Steps
    ...
    % compute gradient
    [U_Curr_Grad,U_Curr] = uq_gradient(Sample_Curr, U, 'forward', 'fixed', 1e-3, -LogPrior(Sample_Curr)-logLKLHD_Curr);
    ...
    for h = 1:LeapfrogSteps
        ...
        outOfBounds = isinf(LogPrior(Sample_Cand));
        % compute gradient
    ...
    if and(mod(ii,nPlotStep) == 0,plotParameters > 0)
        plotHandle = uq_MCMC_plot(plotHandle,ii,plotParameters,Sample);
    end
end
