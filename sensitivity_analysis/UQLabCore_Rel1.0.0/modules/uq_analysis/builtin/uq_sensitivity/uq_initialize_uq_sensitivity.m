...
methods_noinput = {'Correlation','SRC'};
...
if ~isempty(opt.Value)
    ...
    elseif isempty(CurrentInput) && any(strcmpi(Options.Method,methods_noinput))
        inputsample_path = sprintf('Options.%s.Sample.X',Options.Method);
        try % to investigate the input sample
            M = size(eval(inputsample_path),2);
            Internal.M = size(inputsample_path,2);
        catch me
...
CorrParam = CurrentInput.Copula.Parameters - eye(M);
CorrelInputs = (sum(sum(CorrParam~=0)) > 0); % equals 1 if there are more entries than just the diagonal in the copula parameters
...
if Method.Missing || Method.Invalid
    ...
    switch lower(Method.Value)
        ...
        case {'sobol'}
%             if CorrelInputs
%                 error('Warning: Sobol'' sensitivity analysis not possible for correlated inputs! Assuming independence.')
%             else
...
%             end
        ...
        case {'perturbation'}
            if CorrelInputs
                error('Perturbation sensitivity analysis not possible for correlated inputs!')
            else
...
switch lower(Internal.Method)
    ...
    case 'correlation'
        ...
        if isfield(CorrOpts,'Sample') && ~isempty(CorrOpts.Sample)
            ...
            [opt, CorrOpts] = uq_process_option(CorrOpts, 'Sampling', 'mc', 'char');
    ...
    case 'perturbation'
        ...
        if isfield(Options, 'Gradient') &&  ~isfield(Options.Gradient, 'Step')
    ...
    case 'morris'
        ...
        % GridLevels
        [opt, MorrisOpts] = uq_process_option(MorrisOpts, 'GridLevels', M, 'double');
        InitMorrisOpts.GridLevels = opt.Value;
        
        % PerturbationSteps
        % Define the Delta of the perturbation, as the integer c such that
        % Delta = c * 1/(1-p)
        [opt, MorrisOpts] = uq_process_option(MorrisOpts, 'PerturbationSteps', 3, 'double');
    ...
    case 'sobol'
        ...
        if opt_pce.Value || opt_lra.Value
            ...
            InitSobolOpts.CoefficientBased = opt_pce.Value || opt_lra.Value;
        end
    ...
    case 'borgonovo'
        ...
        % Check for a provided Exp. Design
        [opt_ed, BorgonovoOpts] = uq_process_option(BorgonovoOpts,...
            'ExpDesign',struct,'struct');        
        % if the ED is there, there is no need to sample again (we might
    ...
    case 'kucherenko'
        ...
        switch lower(opt.Value)
            case {'standard'}
                InitKucherenkoOpts.Estimator = 'standard';
            case {'modified'}
                InitKucherenkoOpts.Estimator = 'modified';
            case {'alternative'}
                InitKucherenkoOpts.Estimator = 'alternative';
                
            otherwise
                fprintf('\nWarning: Estimator "%s" is not available. Changed to "%s".\n', KucherenkoOpts.Estimator, opt.Default);
                InitKucherenkoOpts.Estimator = opt.Default;
        end
        ...
        % Check for provided samples
        if isfield(KucherenkoOpts,'Samples') && any(isfield(KucherenkoOpts.Samples, {'X1','X2'}))% Samples are provided
            
            % If only X2 is provided, save it as X1
            if all(isfield(KucherenkoOpts.Samples, {'X1','X2'}) == [0,1])
                if Internal.Display > 0
                    fprintf('\nWarning: only sample 2 for the analysis is provided. The sample will be renamed and used as sample 1!\n')
                    fprintf('Please define sample 1 if only one sample is provided.\n')
                end
                [opt,KucherenkoOpts.Samples] = uq_process_option(KucherenkoOpts.Samples,'X2');
                KucherenkoOpts.Samples.X1 = opt.Value;
                if isfield(KucherenkoOpts.Samples,'Y2')
                    [opt,KucherenkoOpts.Samples] = uq_process_option(KucherenkoOpts.Samples,'Y2');
                    KucherenkoOpts.Samples.Y1 = opt.Value;
                end
            end
            % check X1 and Y1
            if isfield(KucherenkoOpts.Samples, 'X1')
                [opt,KucherenkoOpts.Samples] = uq_process_option(KucherenkoOpts.Samples,'X1');
                InitKucherenkoOpts.x1 = opt.Value;
                InitKucherenkoOpts.SampleSize = size(InitKucherenkoOpts.x1,1);
                if isfield(KucherenkoOpts.Samples, 'Y1') % Check for output sample
                    [opt,KucherenkoOpts.Samples] = uq_process_option(KucherenkoOpts.Samples,'Y1');
                    InitKucherenkoOpts.y1 = opt.Value;
                    if size(InitKucherenkoOpts.y1,1) ~= size(InitKucherenkoOpts.SampleSize,1)
                        fprintf('\n\nError: The amount of samples in X1 and Y1 must be equal!\n');
                        error('While initializing the analysis: unequal sample lengths');
                    end
                end
            end
            % check X2 and Y2
            if isfield(KucherenkoOpts.Samples, 'X2')
                [opt,KucherenkoOpts.Samples] = uq_process_option(KucherenkoOpts.Samples,'X2');
                InitKucherenkoOpts.x2 = opt.Value;
                if isfield(KucherenkoOpts.Samples, 'Y2') % Check for output sample
                    [opt,KucherenkoOpts.Samples] = uq_process_option(KucherenkoOpts.Samples,'Y2');
                    InitKucherenkoOpts.y2 = opt.Value;
                    if size(InitKucherenkoOpts.y2,1) ~= size(InitKucherenkoOpts.x2,1)
                        fprintf('\n\nError: The amount of samples in X2 and Y2 must be equal!\n');
                        error('While initializing the analysis: unequal sample lengths');
                    end
                end
                
                % Do not use provided X2 if the alternative estimator is selected
                if strcmp(InitKucherenkoOpts.Estimator,'alternative')
                    clear InitKucherenkoOpts.XX1 InitKucherenkoOpts.YY1
                    if Internal.Display > 0
                        fprintf('\nWarning: For the chosen "%s" estimator X2 will be sampled with the\n', InitKucherenkoOpts.Estimator);
                        fprintf('chosen QMC sampling strategy "%s".\n', InitKucherenkoOpts.Sampling);
                    end
                end
            end
            % check if X1 and X2 have the same size
            if all(isfield(InitKucherenkoOpts,{'x1','x2'})) && ...
                    size(InitKucherenkoOpts.SampleSize,1) ~= size(InitKucherenkoOpts.x2,1)
                fprintf('\n\nError: The amount of samples in X1 and X2 must be equal!\n');
                error('While initializing the analysis: unequal sample lengths');
            end
            
        else % Define SampleSize
            % SampleSize
        ...
        % Set to Internal
        Internal.IndexOpts = InitKucherenkoOpts;
