...
if ~Options.CustomPCE
    ...
    if isfield(Options.PCEOpts.ExpDesign, 'NSamples')
        Results.Cost = Options.PCEOpts.ExpDesign.NSamples; % input sample had to be evaluated
    else
        Results.Cost = 0;
    end
else
    ...
    for oo = 1: size(subsets,1) % index subsets
        ...
        for ii = 1: size(subsets,2) % variables
            ...
            switch oo
                ...
                case 2
                    idx1 = ~workset(:,ii);
                    idx2 = sum(workset~=0,2)==1;
                    idx_final = ~idx1 & ~idx2;      % same as before, but only first condition
                case 3
                    idx_final = ~(workset(:,ii)~=0);% if the first entry of a row is ~=0, not wanted
            end
