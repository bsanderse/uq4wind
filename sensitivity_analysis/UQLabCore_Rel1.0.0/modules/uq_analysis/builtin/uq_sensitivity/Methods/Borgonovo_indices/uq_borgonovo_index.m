...
switch Options.BinStrat
    case 'quantile'
        % In this case all classes get the same amount of samples. This is
        % achieved through working with quantiles.
        % The amount of quantiles equals the amount of bin edges (nbins+1)
        nedges = nbins_x+1;
        Xi_quantiles = linspace(0,1,nedges);
        
        if h_overlap == 0 %%% for no overlap
            % Get the class edges with 'nquant' quantiles
            class_edges = quantile(Xi,Xi_quantiles);
            class_edges(end) = class_edges(end)*1.01;
            
        else %%% in case we have overlap between classes (default)
            % Calculate the shift of the egdes on each side through overlap
            class_width = 1/nbins_x;
            edge_shift = class_width*0.5*h_overlap;
            
            % We need to define lower and upper class edges
            quantiles_low = [0, Xi_quantiles(2:(end-1)) - edge_shift];
            quantiles_up = [Xi_quantiles(2:(end-1)) + edge_shift, 1];
            
            class_edges_low = quantile(Xi,quantiles_low);
            class_edges_up = quantile(Xi,quantiles_up);
        end
            
    case 'constant'
        % In this case all classes have the same, constant width.
        nedges = nbins_x+1;
        % go a bit over the last values to make sure, they're included
        constedges = linspace(min(Xi)*0.99,max(Xi)*1.01,nedges);
        
        if h_overlap == 0 %%% for no overlap
            % Get the class edges with 'nquant' quantiles
            class_edges = constedges;
            
        else %%% in case we have overlap between classes (default)
            % Calculate the shift of the egdes on each side through overlap
            class_width = constedges(2)-constedges(1);
            edge_shift = class_width*0.5*h_overlap;
            
            % With the sift we can define lower and upper class edges
            class_edges_low = [0 constedges(2:(end-1))-edge_shift];
            class_edges_up = [constedges(2:(end-1))+edge_shift 1];
        end
        
end %%% of    switch BinStrat
