...
semilogy(NED, abs(mean_MC/mean_exact-1),...
    '-', 'Linewidth', 2, 'Color', myColors(1,:))
hold on
semilogy(NED, abs(std_MC/std_exact-1),...
    '--', 'Linewidth', 2, 'Color', myColors(1,:))
semilogy(NED, abs(mean_PCE/mean_exact-1),...
    '-', 'Color', myColors(2,:))
semilogy(NED, abs(std_PCE/std_exact - 1),...
...
xlabel('$\mathrm N$')
legend({'$\mathrm{|\hat{\mu}_Y^{MC} / \mu_Y - 1 |}$',...
