...
% the experimental design is automatically generated
% and the corresponding model responses are computed.

%%
% Specify the sampling strategy and the number of sample points for the
% experimental design:
...
%%
% Use the hybrid genetic algorithm (GA)-based for the hyperparameters
% calibration:
...
%%
% Create the Kriging metamodel object:
...
%
% Generate a validation sample of size $3$:
Xval = uq_getSample(3);

%%
% Evaluate the full computational model at the validation sample points:
...
legend({'$\mathrm{\mathcal{M}(\mathbf{x}^{(1)})}$',...
    ...
    'FontSize', 12, 'Interpreter', 'latex', 'Location', 'north')
