...
% has to be specified for the LRA metamodel.
...
hold on
uq_histogram(YLRA, 'FaceColor', myColors(2,:))
hold off

uq_setInterpreters(gca)
xlabel('$\mathrm{Y}$')
ylabel('Counts')
legend({'True model response', 'LRA prediction'}, 'Location', 'northwest')
...
%%
% Print the validation and cross-validation errors:
fprintf('LRA metamodel validation error:       %5.4e\n', myLRA.Error.Val)
fprintf('LRA metamodel cross-validation error: %5.4e\n',...
