...
% 
% For details, see: Sudret, B. (2008). "Analytical derivation of
% the outcrossing rate in time-variant reliability problems,"
% Structure and Infrastructure Engineering, 4 (5), 353-362 (Section 5).
...
myFORM = uq_createAnalysis(FORMOptions);

%% 7 - ESTIMATION OF THE OUTCROSSING RATE
%
% Using the same equations as in UQ_EXAMPLE_RELIABILITY_04_PARALLELSYSTEM,
% the parallel system failure probability can be estimated as follows:
...
tt = 0:1:50;
vv = omega_0 * PSI(-b/(omega_0*sigmaS)) * (sigmaS/sqrt(sigmaR^2+sigmaS^2)) *...
    normpdf((muR-b*tt-muS)/sqrt(sigmaR^2+sigmaS^2));

%%
% Use the same limit state function and FORM options as before:
LSiOpts = LSOpts;
FORMiOptions = FORMOptions;
FORMiOptions.Display = 0;
...
for ii = 1:length(tt)
    ...
    myFORMi = uq_createAnalysis(FORMiOptions);
...
uq_plot(tt,vv,'b-')
hold on
uq_plot(tt(1:2:end), vv_FORM(1:2:end), 'or')
hold off
grid on
xlim([0 50])
ylim([0 9e-4])
set(gca, 'XTick', 0:10:50, 'YTick', 0:1e-4:1e-3)
l = uq_legend('Analytical result','PHI2 approximation');
