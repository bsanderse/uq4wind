...
[I,~] = imread('SimplySupportedBeam_Bayesian.png');
image(I)
axis equal
set(gca, 'visible', 'off')
...
uq_figure('Position', [50 50 500 400]) 
[I,~] = imread('ElongationBeam_Bayesian.png');
image(I)
axis equal
set(gca, 'visible', 'off')
...
%
% * $E \sim \mathcal{LN}(\mu_E = 30000, \sigma_E = 4500)$
% * $p \sim \mathcal{LN}(\mu_p = 0.012, \sigma_p = 0.003)$
% * $P \sim \mathcal{LN}(\mu_P = 0.05, \sigma_P = 0.001)$
%
...
%
% * $\sigma^2_{\mathrm{1}} \sim \mathcal{U}(0, 0.01)$
% * $\sigma^2_{\mathrm{2}} \sim \mathcal{U}(0, 0.01)$
%
...
SigmaOpts.Marginals(1).Parameters = [0 0.01];
...
% algorithm is employed in this example, with $700$ iterations
% and $100$ parallel chains:
Solver.Type = 'MCMC';
Solver.MCMC.Sampler = 'AIES';
Solver.MCMC.Steps = 700;
...
% Post-process the posterior sample using |uq_postProcessInversion|:
myBayesianAnalysis = uq_postProcessInversion(myBayesianAnalysis);
