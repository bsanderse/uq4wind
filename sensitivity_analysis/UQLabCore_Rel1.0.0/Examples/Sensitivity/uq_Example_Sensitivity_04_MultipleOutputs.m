...
%%
% Note that the total cost will equal (M+2)*SampleSize
% for sampling-based Sobol' indices calculation 
...
%%
% Plot the total indices for all the output components:
uq_figure('Position', [50 50 500 400])
...
ynames = {'$\mathrm Y_1$', '$\mathrm Y_2$', '$\mathrm Y_3$',...
    '$\mathrm Y_4$', '$\mathrm Y_5$', '$\mathrm Y_6$', '$\mathrm Y_7$',...
    '$\mathrm Y_8$', '$\mathrm Y_9$'};
set(gca, 'XTick', 1:length(SobolResults.VariableNames),...
    'XTickLabel', SobolResults.VariableNames, 'FontSize', 14, 'Box', 'on')
uq_legend(ynames', 'Location', 'northwest', 'FontSize', 12,...
    'Interpreter', 'latex')

%%
% Plot the first-order indices for all the output components:
uq_figure('Position', [50 50 500 400])
