...
for ii = RunList
    ...
    for tt = 1:length(P.Template)
        ...
        expr_bounds = strcat(P.Marker{1} ,'(\w.*)', P.Marker{3}) ;
        while ~feof(ftemp)  % Read the template until the end of file is reached
            ...
            for jj = 1 : size(X,2)
                ...
                if ~isempty(str_expr)
                    % Only the expressions will be replaced in this section
                    str_temp = str_expr{1};

                    if length(str_temp) - 2 > length(P.Marker{2})+4
                        ...
                        for ll = 1:length(str_split)
                            if length(str_split{ll}) - 2 > length(P.Marker{2})+4
                                ...
                                % 4 (number of digits) (for instance > 5 for X0001)
                                % An expression starting with < and ending with >
                                % has been found
                                ...
                                str = replace( str,  str_split_original{ll}, num2str(expr_value)) ;
                            end
                        end
                    end
                ...
                % Handling of expressions in the input file 
...
%                                 param_value_as_str = num2str( X(ii,kk), P.Format{kk} ) ;
%                             end
%                             str_expr_updated = regexprep( str_expr_updated, ...
%                                 strcat(P.Marker{2}, num2str(kk,'%04u')), ...
%                                 param_value_as_str ) ;
%                         end
%                         % Now evaluate the expression
%                         expr_value = eval(str_expr_updated) ;
%                         % Finally replace it im the string
%                         str = regexprep( str, expr_value, param_value_as_str ) ;
%                     end
                
                % Now search for the output file name and if found,
                % replaced it by the numbered one
                % I DON'T GET THIS (Moustapha, 04/07/2019)
                if jj == 1
                    % Replace only once
    ...
    switch (P.Archiving.Action)
        case 'save'
            ...
            for tt = 1:length(newfiles)
                try
                    movefile(fullfile(P.ExecutionPath,newfiles{tt}),AuxFolder);
                catch
                    % do nothing
                end
            end
