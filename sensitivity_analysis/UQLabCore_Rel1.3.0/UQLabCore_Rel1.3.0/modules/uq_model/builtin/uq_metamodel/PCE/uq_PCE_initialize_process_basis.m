...
if ~strcmpi(current_model.Internal.Method,'custom')
    ...
    if isfield(TruncOpts,'Custom') && ~isempty(TruncOpts.Custom)
        ...
        maxdeg = max(max(sum(TruncOpts.Custom,2)))+1;
    ...
    if isfield(current_model.Options, 'Degree')
        ...
        maxdeg = max(sum(current_model.PCE(pce_idx).Basis.Indices,2))+1;
    end
end
...
if (~PolyTypes.Missing)
    ...
    % polynomial names do not define anything. uq_poly_marginals will need
    % information about the constant ones. Namely where the constant is
    % fixed for 'zero' polynomial type. We also treat that here.
