...
for oo = 1:NOuts    
    ...
    if B > 0
        % initialization 
        if ~exist('Bstr', 'var')
            Bstr.Total.CI = zeros([size(TotalSobolIndices') 2]);
            Bstr.Total.Mean = zeros(size(TotalSobolIndices'));
            Bstr.Total.ConfLevel = zeros(size(TotalSobolIndices'));
        ...
        Bstr.Total.CI(:,oo,:) = Btotal.CI';
        Bstr.Total.Mean(:,oo) = Btotal.Mean;
        Bstr.Total.ConfLevel(:,oo) = Btotal.ConfLevel;
        % First order:
        if Options.Sobol.Order >= 1
            ...
            Bstr.AllOrders{1}.CI(:,oo,:) = B1st.CI';
            Bstr.AllOrders{1}.Mean(:,oo) = B1st.Mean;
            Bstr.AllOrders{1}.ConfLevel(:,oo) = B1st.ConfLevel;
