...
% >> X = 4+randn(100,1);
% >> Marginals(1).Type = 'auto';
% >> Marginals(1).Inference.Families = {'Gaussian', 'Gumbel'};
% >> uq_infer_marginals(X, Marginals)
%
% % Fit marginal by kernel smoothing; specify bounds and kernel bandwidth
% >> X = rand(100,1);
% >> Marginals(1).Type = 'auto';
% >> Marginals(1).Inference.Families = 'ks';
% >> Marginals(1).Options.Bounds = [0 1];
% >> Marginals(1).Options.Bandwidth = 0.1;
% >> uq_infer_marginals(X, Marginals)
...
% Redefine function ksdensity such that is uses the specified (or default)
% input kernel parameters
[Kernels{1:M}] = deal('normal');    % default Kernels: all normal
[KernelBandwidths{1:M}] = deal([]);   % default bandwiths: not provided
[Supports{1:M}] = deal('unbounded');  % default supports: unbounded
if isfield(Marginals, 'Options')    % overwrite the defaults, if other
    for ii = 1:M                    % values have been specified
        if isfield(Marginals(ii).Options, 'Kernel')
            Kernels{ii} = lower(Marginals(ii).Options.Kernel);
            if strcmpi(Kernels{ii}, 'gaussian')
                Kernels{ii} = 'normal';
            elseif strcmpi(Kernels{ii}, 'triangular')
                Kernels{ii} = 'triangle';
            end
        end
        if isfield(Marginals(ii).Options, 'BandWidth')
            KernelBandwidths{ii} = Marginals(ii).Options.BandWidth;
        end
        if isfield(Marginals(ii), 'Bounds')
            Supports{ii} = Marginals(ii).Bounds;
        end
    end       
end

myksd = @(x,y,ii) ksdensity(x, y, 'kernel', Kernels{ii}, 'support', ...
    Supports{ii}, 'bandwidth', KernelBandwidths{ii}, 'function', 'pdf');
...
for jj = 1:M                                 % For each jj-th marginal,
    ...
    elseif isa(Marginals(jj).Type, 'char') && ...
        ...
        % Transform X into Z~N(0,1) and compute K-S test statistic and pval
        Z = uq_all_invcdf(uq_all_cdf(x, myMarginals(jj)), ...
            uq_StdNormalMarginals(1)); 
        [~, S.KSpvalue, S.KSstat] = kstest(Z);
    ...
    % CASE 2: inference to be performed
        ...
        for ll = 1:l % For each family of distribs to fit on x...  
            ...
            % Case: family to fit is kernel density
            elseif strcmpi(DistNames_Std(Family_ll), 'ks')
                lls_margins{jj}(ll) = sum(log(myksd(x, x, jj)));
                params = x;
            ...
            Z = uq_all_invcdf(uq_all_cdf(x, FittedMarginal_ll), ...
                uq_StdNormalMarginals(1)); 
            [~, S.KSpvalue, S.KSstat] = kstest(Z);
