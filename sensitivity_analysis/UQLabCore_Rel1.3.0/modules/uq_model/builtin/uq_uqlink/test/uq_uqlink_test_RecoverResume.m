...
load(fullfile(Mopts.ExecutionPath,'myBeam.mat'), 'uq_ProcessedY') ;
% Put the second output to NaN
uq_ProcessedY(2,:) = NaN ;
%Save back the modified input
save(fullfile(Mopts.ExecutionPath,'myBeam.mat'), 'uq_ProcessedY', '-append')  ;
...
% Load Processed input/output and modify them
load(fullfile(Mopts.ExecutionPath,'myBeam.mat'), 'uq_ProcessedX', 'uq_ProcessedY') ;
% Remove the third output to pretend that it is not 
uq_ProcessedX =  uq_ProcessedX(1:2,:) ;
uq_ProcessedY =  uq_ProcessedY(1:2,:) ;
%Save back the modified input
save(fullfile(Mopts.ExecutionPath,'myBeam.mat'), 'uq_ProcessedX', 'uq_ProcessedY','-append') ;
...
try
    load(fullfile(Mopts.ExecutionPath,'myBeam.mat'), 'uq_AllX') ;
