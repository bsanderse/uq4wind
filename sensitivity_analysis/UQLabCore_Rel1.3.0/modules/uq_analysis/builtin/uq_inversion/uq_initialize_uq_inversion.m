...
if ~CUSTOM_LIKELI
    ...
    if isempty(Internal.ModelConstInfo.idConst) % without constants
      ...
      for ii = 1:Internal.nForwardModels
        ...
        discrConst = any(bsxfun(@eq,ModelConstInfo.idConst,Internal.ForwardModel(ii).PMap'));
        discrFull = any(bsxfun(@eq,ModelConstInfo.idFull,Internal.ForwardModel(ii).PMap'));
...
switch upper(Opt.Value.Type)
    case 'NONE'
        ...
        Opt.Value.LogPrior = @(x) uq_evalLogPDF(x,Internal.FullPrior);
    ...
    case 'MCMC'
        ...
        switch upper(Opt.Value.MCMC.Sampler)
            ...
            case 'HMC'
                ...
                if and(all(size(mass)==1), mass > 0)
                    %positive scalar
                    Opt.Value.MCMC.Mass = mass*eye(length(Internal.FullPrior.Marginals));
                else 
                    %compute cholesky decomposition to make
        ...
        if size(Opt.Value.MCMC.Seed,2) > length(Internal.FullPrior.Marginals)
            ...
            Opt.Value.MCMC.Seed(:,Internal.ModelConstInfo.idConst,:) = [];
        end
