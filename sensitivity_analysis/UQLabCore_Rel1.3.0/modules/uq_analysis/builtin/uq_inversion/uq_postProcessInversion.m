...
if ~strcmp(module.Type, 'uq_inversion')
    error('uq_postProcessInversion only operates on objects of type ''Inversion''') 
end
...
if nargin > 1
    % vargin given
    parse_keys = {'burnin', 'badchains', 'pointestimate','correlation',...
...
if priorPredictive_flag
    ...
    for mm = 1:nForwardModels
        ...
        if pointEstimate_flag
            ...
            pointParamCurrModel = pointParam(:,PMapCurr);
            model(mm).pointEstimateRun = uq_evalModel(module.Internal.ForwardModel_WithoutConst(mm).Model,pointParamCurrModel);
        end
        %prior Runs
    ...
    for ii = 1:module.Internal.nDataGroups
        ...
        for mm = 1:nForwardModels
            if any(MOMap(1,:)==mm)
                ...
                OMapCurr = MOMap(2,MIndex);
                %predictive runs
                model(mm).priorPredRuns(:,OMapCurr) = priorPredRunsCurr(:,MIndex);
...
if posteriorPredictive_flag
    ...
    for mm = 1:nForwardModels
        ...
        if pointEstimate_flag
            %remove discrepancy terms
            pointParamModel = pointParam(:,modelIndices);
    ...
    for ii = 1:module.Internal.nDataGroups
        ...
        for mm = 1:module.Internal.nForwardModels
            ...
            OMapCurr = MOMap(2,MIndex);
            if any(MIndex)
                postRunsCurr(:,MIndex) = model(mm).postRuns(:,OMapCurr);
            end
        end
        ...
        for mm = 1:nForwardModels
            if any(MOMap(1,:)==mm)
                ...
                OMapCurr = MOMap(2,MIndex);
                %predictive runs
                model(mm).postPredRuns(:,OMapCurr) = postPredRunsCurr(:,MIndex);
                %data related to current model
                for kk = 1:length(OMapCurr)
                    model(mm).data(OMapCurr(kk)).y = ...
                        [model(mm).data(OMapCurr(kk)).y;yCurr(:,kk)];
