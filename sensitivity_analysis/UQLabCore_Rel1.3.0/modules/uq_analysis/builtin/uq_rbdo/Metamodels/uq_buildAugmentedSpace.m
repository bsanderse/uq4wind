    ...
    switch lower(current_analysis.Internal.AugSpace.Method)
        case {'hypercube'}
            ...
            for ii = 1:M_z
                ...
                % Get parameters of f_{Z_ii}
%                 if isfield(current_analysis.Internal.Input.EnvVar, 'Parameters')
...
%                     TempMarginal = uq_MarginalFields(TempMarginal) ;
%                 end
                % Get Zmin and Zmax for up to three parameters of the
                % distribution
        ...
        case {'hybrid'}
            ...
            for ii = 1:length(current_analysis.Internal.Input.EnvVar)
                TempMarginal.Type = current_analysis.Internal.Input.EnvVar.Marginals(ii).Type;
                % Get parameters of the distribution
%                 if isfield(current_analysis.Internal.Input.EnvVar,'Parameters')
                    TempMarginal.Parameters = current_analysis.Internal.Input.EnvVar(ii).Parameters;
%                 else
%                     TempMarginal.Moments = current_analysis.Internal.Input.EnvVar(ii).Moments;
%                     TempMarginal = uq_MarginalFields(TempMarginal) ;
%                     TempMarginal = rmfield(TempMarginal,'Moments') ;
