...
OptimKnownMethods = {'GA','HGA', 'CCMAES','HCCMAES', 'IP','SQP','intCCMAES'} ;
% Known optimization methods with respect to toolbox
OptimKnownOptimToolboxMethods = {'IP','SQP','HGA','HCE','HCCMAES'} ;
...
OptimReqFields.INTCCMAES   = {'Display', 'MaxIter','TolFun'} ;
...
if isfield(Options, 'Input')
    ...
    if isfield(Options.Input, 'EnvVar')
        ...
        if ~isa(envInput.Value,'uq_input')
            error('The provided input model for the environmental variables is not a valid UQLab INPUT model!') ;
        end

        Internal.Input.EnvVar = envInput.Value ;
        
        % NUmber of environmental variables
        M_z = length(envInput.Value.Marginals) ;
...
if isMFILE + isMSTRING + isMHANDLE + isMODEL < 1
    ...
    if isMODEL
        ...
        if ~isa(current_analysis.Internal.Cost.Options.Model,'uq_default_model') &&...
                ~isa(current_analysis.Internal.Cost.Options.Model,'uq_uqlink') && ...
                ~isa(current_analysis.Internal.Cost.Options.Model,'uq_metamodel')
...
current_analysis.Internal.LimitState.Model = optCons.Value ;
if ~isa(current_analysis.Internal.LimitState.Model,'uq_default_model') &&...
        ~isa(current_analysis.Internal.LimitState.Model,'uq_model') && ...
        ~isa(current_analysis.Internal.LimitState.Model,'uq_uqlink') && ...
        ~isa(current_analysis.Internal.LimitState.Model,'uq_metamodel')
...
if isfield(Options,'SoftConstraints')
    ...
    if isMODEL
        % The user gave a model object. Make sure that the model is of type
        % default_model, UQLink or Metamodel
        if ~isa(current_analysis.Internal.SoftConstraints.Options.Model,'uq_default_model') &&...
                ~isa(current_analysis.Internal.SoftConstraints.Options.Model,'uq_uqlink') && ...
                ~isa(current_analysis.Internal.SoftConstraints.Options.Model,'uq_metamodel')
